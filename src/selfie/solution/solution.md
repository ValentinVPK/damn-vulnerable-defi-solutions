...
- This level was about exploiting a governance protocol (DAO) by claiming large portion of the voting power
- This was achieved by taking a flash loan with the voting tokens and using them to execute transactions before returning them
...
// Solution test
    function test_selfie() public checkSolvedByPlayer {
        AttackSelfiePool attackSelfiePool = new AttackSelfiePool(address(pool), address(token), address(governance));

        uint256 targetActionId = attackSelfiePool.attack(recovery);
